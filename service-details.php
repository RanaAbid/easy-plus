...
// Get service slug from URL (primary method, ID for backward compatibility only)
$serviceSlug = isset($_GET['slug']) ? sanitizeInput($_GET['slug']) : '';
$serviceId = isset($_GET['id']) ? intval($_GET['id']) : 0;
$service = null;

// Always try slug first if provided
if (!empty($serviceSlug) && function_exists('getServiceBySlug')) {
    $service = getServiceBySlug($link, $serviceSlug);
} elseif ($serviceId > 0) {
    // Backward compatibility: if ID is provided, get by ID and redirect to slug
